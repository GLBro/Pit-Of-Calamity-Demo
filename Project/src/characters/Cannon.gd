...
signal fire_cannon(bullet, position, direction, speed, damage)

func _ready() -> void:
	defineEnemy(75,5,"Cannon")
	$AnimatedSprite.play("walking")
	get_parent().get_parent().get_parent().cannon_fired()
...
func fire_cannons():
	var left_ball_instance = Ball.instance()
	emit_signal("fire_cannon", left_ball_instance, $Left_Cannon.global_position, Vector2(-1,0), 5, 15)
	var right_ball_instance = Ball.instance()
	emit_signal("fire_cannon", right_ball_instance, $Right_Cannon.global_position, Vector2(1,0), 5, 15)
	print("emitted")
